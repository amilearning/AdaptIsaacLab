...
- Throughout the repository, the name `AdaptIsaacLab` only serves as an example and we provide a script to rename all the references to it automatically:

```bash
# Enter the repository
cd IsaacLabExtensionTemplate
# Rename all occurrences of AdaptIsaacLab (in files/directories) to your_fancy_extension_name
...
```bash
python -m pip install -e source/AdaptIsaacLab
```
...
We provide an example UI extension that will load upon enabling your extension defined in `source/AdaptIsaacLab/AdaptIsaacLab/ui_extension_example.py`.
...
```json
{
    "python.analysis.extraPaths": [
        "<path-to-ext-repo>/source/AdaptIsaacLab"
    ]
}
```
